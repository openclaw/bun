...
describe("Bun.ModuleGraph — CommonJS surface per graph", () => {
  ...
  describe.each(["#", "?"])("CommonJS filename delimiter %s", delimiter => {
    test.skipIf(isWindows && delimiter === "?")(
      "encoded CommonJS query keys are evicted after a throw only in the owning graph",
      async () => {
        const filename = `retry${delimiter}encoded.cjs`;
        const d = fixture({
          "state.cjs": `module.exports = { attempts: 0 };`,
          [filename]: `const state = require("./state.cjs"); if (++state.attempts === 1) throw new Error("first attempt"); module.exports = state.attempts;`,
          "user.cjs": `
          const { pathToFileURL } = require("node:url");
          const { join } = require("node:path");
          const target = pathToFileURL(join(__dirname, ${JSON.stringify(filename)})).href + "?v=1";
          exports.tryOnce = () => { try { return require(target); } catch (error) { return error.message; } };
          exports.cached = () => Object.hasOwn(require.cache, require.resolve(target));
          exports.attempts = () => require("./state.cjs").attempts;
        `,
        });
        using first = new ModuleGraphClass();
        using second = new ModuleGraphClass();
        const a = await first.import(join(d, "user.cjs"));
        const b = await second.import(join(d, "user.cjs"));
        const host = createRequire(join(d, "host.cjs"))(join(d, "user.cjs"));
        expect([a.attempts(), b.attempts(), host.attempts()]).toEqual([0, 0, 0]);
        expect([a.tryOnce(), a.cached()]).toEqual(["first attempt", false]);
        expect([a.tryOnce(), a.cached(), a.attempts()]).toEqual([2, true, 2]);
        expect([b.attempts(), b.cached(), host.attempts(), host.cached()]).toEqual([0, false, 0, false]);
        expect([b.tryOnce(), b.cached(), host.tryOnce(), host.cached()]).toEqual([
          "first attempt",
          false,
          "first attempt",
          false,
        ]);
        expect([b.tryOnce(), host.tryOnce(), a.tryOnce()]).toEqual([2, 2, 2]);
        expect([a.attempts(), b.attempts(), host.attempts()]).toEqual([2, 2, 2]);
      },
    );
  });

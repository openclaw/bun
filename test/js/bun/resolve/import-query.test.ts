...
describe.each(["?v=1", "#v1", "?#v1", "?"])("mock URL suffix %s", suffix => {
  test.concurrent("mock.module with a file:// URL + query string registers under the key import() uses", async () => {
    using dir = tempDir("import-query-file-url-mock", {
      ...
      mock.module(base + ${JSON.stringify(suffix)}, () => ({ default: "MOCKED", url: "mocked" }));
      const m1 = await import(base + ${JSON.stringify(suffix)});
...
describe.each(["target?copy.mjs", "literal?dir/target.mjs"])("encoded delimiter in %s", filename => {
  ...
  `;
    using dir = tempDir("import-query-file-url-literal-question", {
      "dep.mjs": `export const value = ${filename.includes("/") ? 0 : 42};`,
      "literal?dir/dep.mjs": `export const value = 42;`,
      [filename]: targetSource,
    });
    const target = Bun.pathToFileURL(path.join(String(dir), filename)).href;
    await Bun.write(
      path.join(String(dir), "entry.mjs"),
      [
...
describe.each(["target?copy.cjs", "literal?dir/target.cjs"])("CommonJS filename %s", filename => {
  test.concurrent.skipIf(isWindows)(
    ...
    async () => {
      using dir = tempDir("require-file-url-literal-question", {
        "dep.cjs": `module.exports = ${filename.includes("/") ? 0 : 42};`,
        "literal?dir/dep.cjs": `module.exports = 42;`,
        [filename]: `module.exports = {
        dep: require("./dep.cjs"),
        filename: require("node:path").basename(__filename),
        dirname: require("node:path").basename(__dirname),
        lookupRelative: require("node:path").basename(require.resolve.paths("./dep.cjs")[0]),
        lookupBare: require("node:path").basename(require("node:path").dirname(require.resolve.paths("pkg")[0])),
        modulePaths: require("node:path").basename(require("node:path").dirname(module.paths[0])),
      };`,
      ...
      const { pathToFileURL } = require("node:url");
      const literal = pathToFileURL(path.join(__dirname, ${JSON.stringify(filename)})).href;
    ...
    `,
      ...
      const dirname = filename.includes("/") ? "literal?dir" : path.basename(String(dir));
      const literalResult = {
        dep: 42,

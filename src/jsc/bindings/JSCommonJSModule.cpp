...
bool JSCommonJSModule::load(JSC::VM& vm, Zig::GlobalObject* globalObject)
{
    ...
    if (auto exception = scope.exception()) {
        ...
        bool wasRemoved = requireMapOf(globalObject, moduleGraph())->remove(globalObject, requireMapKey);
        RETURN_IF_EXCEPTION(scope, false);
...
JSC_DEFINE_HOST_FUNCTION(requireResolvePathsFunction, (JSGlobalObject * globalObject, CallFrame* callframe))
{
    ...
    RETURN_IF_EXCEPTION(scope, {});
    Bun::PathResolveModule parent = { .paths = nullptr, .filename = filename, .pathsArrayLazy = true, .filenameIsModuleKey = boundModule->filenameIsModuleKey };
...
JSC_DEFINE_CUSTOM_GETTER(getterPaths, (JSC::JSGlobalObject * globalObject, JSC::EncodedJSValue thisValue, JSC::PropertyName))
{
    ...
    if (!thisObject->m_paths) {
        ...
        JSValue paths = JSValue::decode(Resolver__nodeModulePathsJSValue(&filenameStr, globalObject, true, thisObject->filenameIsModuleKey));
        RETURN_IF_EXCEPTION(scope, {});
        ...
        JSC::EncodedJSValue value, JSC::PropertyName propertyName))
{
    ...
    thisObject->filenameIsModuleKey = false;
    return true;
}
    ...
    JSValue parent)
{
    ...
    out->filenameIsModuleKey = !decodedFileURLKey;
                    ...
                    if (auto* moduleObject = dynamicDowncast<JSCommonJSModule>(entry)) {
                        if (!moduleObject->hasEvaluated) {
                            ...
                            if (auto exception = scope.exception()) {
                                ...
                                // On error, remove the module from the require map
                                // so that it can be re-evaluated on the next require.
                                requireMap->remove(globalObject, keyValue);
                                RETURN_IF_EXCEPTION(scope, void());

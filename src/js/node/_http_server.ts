...
interface Server extends NodeHTTPServer {
  ALPNProtocols?: import("node:tls").TlsOptions["ALPNProtocols"];
  ALPNCallback?: import("node:tls").TlsOptions["ALPNCallback"];
  _SNICallback?: import("node:tls").TlsOptions["SNICallback"];
...
Server.prototype.listen = function () {
  ...
  let port, host;
  let socketPath;
  const initialTls = this[tlsSymbol];
  let tls = initialTls;
  ...
  if (host) {
    lookupListenAddress(host, (err, address) => {
      ...
      try {
        // setSecureContext can replace the server's context during lookup.
        const currentTls = server[tlsSymbol];
        startServerListen(
          server,
          currentTls !== initialTls ? currentTls : tls,
          port,
          address,
          socketPath,
          serverNameHost,
